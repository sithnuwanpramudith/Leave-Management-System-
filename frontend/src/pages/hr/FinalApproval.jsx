...
import { CheckCircle, XCircle, AlertCircle, Loader, RefreshCw, MessageSquare } from 'lucide-react';
...
const FinalApproval = () => {
  ...
  const fetchLeaves = async () => {
    setLoading(true);
    try {
      const data = await leaveAPI.getAll('MANAGER_APPROVED');
      setLeaves(data);
    } catch {
  ...
  const handleAction = async (action) => {
    setActing(true);
    try {
      await leaveAPI.hrAction(actionModal.id, action, comments);
      showSuccess(`Leave request ${action}d by HR.`);
      setActionModal(null);
      setComments('');
      fetchLeaves();
    } catch (e) {
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">These leave requests have been approved by managers and await your final decision.</p>
        </div>
        ...
        : leaves.length === 0
          ? (
            <div className="glass-card p-12 text-center">
              ...
              <p className="text-slate-400">No leaves awaiting final approval.</p>
            </div>
          )
          : (
            <div className="space-y-4">
              {leaves.map(leave => (
                <motion.div key={leave.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-blue-500/10 text-blue-400 flex items-center justify-center font-bold text-lg shrink-0">
                    ...
                    <div>
                      ...
                      {leave.manager_comments && <p className="text-xs text-slate-400 mt-1 italic">Manager: "{leave.manager_comments}"</p>}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 shrink-0">
                    <button onClick={() => { setActionModal(leave); setComments(''); }} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold bg-primary-500/10 text-primary-400 hover:bg-primary-500/20 transition">
                      <MessageSquare size={15} /> Final Review
                    </button>
                  </div>
                </motion.div>
              ))}
        ...
        {actionModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 backdrop-blur-sm bg-black/50">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="glass-card w-full max-w-md p-8">
              <h3 className="text-xl font-bold mb-2">Final HR Review</h3>
              <div className="p-3 bg-white/5 rounded-xl mb-4 text-sm space-y-1">
                ...
                {actionModal.manager_comments && <p><span className="text-slate-500">Manager Notes:</span> {actionModal.manager_comments}</p>}
              </div>
              <div className="space-y-2 mb-6">
                <label className="text-xs font-bold text-slate-400 uppercase">HR Comments (optional)</label>
                <textarea rows={3} className="input-field resize-none" placeholder="Add HR comments..." value={comments} onChange={e => setComments(e.target.value)} />
              </div>
